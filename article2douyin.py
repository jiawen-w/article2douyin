...
【画面匹配 · 视频必须严格按出现顺序，这是硬规则】：
本文有 {clip_count} 条短视频素材，它们在文章里是有先后顺序的（清单里视频1→视频{clip_count}就是出场顺序）。请这样组织分镜：
1. 开头钩子镜（1~2 个）：用配图或纯文字，不要用视频。
2. 中间「逐条展示」段：严格按视频清单顺序，每条视频写恰好 1 个分镜——第 1 个展示镜就讲【视频1】画面里发生的事、clip 填 1；第 2 个展示镜讲【视频2】、clip 填 2……依次到视频{clip_count}。clip 编号必须 1,2,3… 连续递增，绝不许跳号、换序或漏掉某条视频。每镜的口播必须真实对应该条视频的画面内容（看清单描述写），不能用一句泛泛的过场话占掉一个视频位。
3. 结尾镜（1~2 个）：总结+互动钩子，用配图或纯文字，不要用视频。
也就是说：有 {clip_count} 条视频，中间就正好有 {clip_count} 个 clip 分镜，clip 字段依次是 1,2,…,{clip_count}。
配图(image)用于开头/结尾/补充镜：填最贴合的配图编号（1~{image_count}），此时 clip 填 null；没合适的就 image 和 clip 都填 null（用上一个画面兜底）。
会议海报、报名页、品牌LOGO、二维码、嘉宾名单这类广告图一律不要选。
...
def align_videos_by_order(scenes, videos):
    """兜底对齐：把所有用视频的分镜，按视频在文章里的出现顺序依次重排，
    保证第 k 个视频镜头一定用第 k 条视频，杜绝 AI 选错导致的图文错位。"""
    avail = [i + 1 for i, v in enumerate(videos) if v]     # 可用视频序号(出现顺序)
    k = 0
    for s in scenes:
        if s.get("clip"):
            if k < len(avail):
                s["clip"], s["image"] = avail[k], None
                k += 1
            else:
                s["clip"] = None                            # 视频不够，退回用图/兜底
    if k:
        print(f"[align] 视频镜头按出现顺序对齐：{k} 个镜头 → 视频 {avail[:k]}")
    return scenes
...
def run(source):
    ...
    # 兜底：视频镜头一律按文章出现顺序对齐，防止 AI 选错素材
    align_videos_by_order(data["scenes"], videos)
