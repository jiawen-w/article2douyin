...
【硬性禁止】：
- 绝对不要出现原文作者、编辑、记者的人名或署名（如"作者是XX""XX编辑""XX报道"），也不要提媒体出处署名。只讲事实和观点本身。

【配图匹配】：每个分镜指定一张最贴合该句内容的配图编号（image 字段，1~{image_count}）。必须按上面的配图描述选，内容对不上就填 null。同一张图别连续重复用。
...
CAPTION_PROMPT = """用一句话（≤20字）客观描述这张图片画面里有什么（人物/产品/图表/文字/场景），用于短视频选配图。只输出描述本身，不要任何前缀。"""
...
def parse_url(url, workdir):
    ...
    return parse_html_text(resp.text, url, workdir)
...
def ai_caption_images(images):
    ...
    for idx, path in enumerate(images, 1):
        ...
        try:
            ...
            cap = msg.content[0].text.strip().replace("\n", " ")[:30]
            captions[idx] = cap
            print(f"[vision] 图{idx}: {cap}")
        except Exception as e:
            print(f"[vision] 图{idx} 描述失败：{e}")
    return captions


def ai_make_script(article, image_count, captions):
    ensure_package("anthropic")
    import anthropic

    if captions:
        caption_block = "\n".join(f"图片{i}：{captions.get(i, '（无法识别）')}"
...
def pick_image(scene, images):
    """按 AI 给的编号取图，无效则返回 None。"""
    idx = scene.get("image")
    if idx and 1 <= idx <= len(images) and images[idx - 1]:
        return images[idx - 1]
    return None


def build_scene_clip(image, mp3, dur, index, workdir):
    """单镜视频：图片模糊填充 + Ken Burns（奇数镜推近、偶数镜拉远）+ 该镜音频。"""
    out = workdir / "clips" / f"clip_{index:02d}.mp4"
    out.parent.mkdir(exist_ok=True)
    total = dur + SCENE_PAD
    frames = int(total * FPS) + 1

    # 缓慢线性缩放；中心恒定 + 高倍超采样 → 消除 zoompan 像素抖动
    zmax = 1.0 + KB_ZOOM
    if index % 2 == 1:                       # 奇数镜：从 1.0 缓推到 zmax
        zoom_expr = f"min({zmax:.4f},1.0+{KB_ZOOM:.4f}*on/{frames})"
    else:                                    # 偶数镜：从 zmax 缓拉到 1.0
...
def run(source):
    ...
    article, images = parse_article(source, tmp_work)
    if len(article.strip()) < 50:
        raise ValueError("正文太短，可能解析失败，请检查来源")
    print(f"[parse] 正文 {len(article)} 字，配图 {len([p for p in images if p])} 张")

    # 2) 视觉识图：给每张图配内容描述，让 AI 能按内容选图
    print("[vision] AI 识别配图内容 ...")
    captions = ai_caption_images(images)

    # 3) AI 生成抖音脚本（带配图描述，确保图文对应）
    data = ai_make_script(article, len(images), captions)
    ...
    if tmp_work != workdir and (tmp_work / "images").exists():
        (workdir / "images").mkdir(exist_ok=True)
        new_images = []
        for p in images:
            if p:
                np_ = workdir / "images" / p.name
                np_.write_bytes(p.read_bytes())
                new_images.append(np_)
            else:
    ...
    for i, (scene, (mp3, dur)) in enumerate(zip(data["scenes"], audio), 1):
        img = pick_image(scene, images) or last_img
        last_img = img or last_img
        clip, total = build_scene_clip(img, mp3, dur, i, workdir)
        clips.append(clip)
        durations.append(total)
        print(f"[clip] 镜{i} 完成（{total:.1f}s，{'有图' if img else '纯底色'}）")
